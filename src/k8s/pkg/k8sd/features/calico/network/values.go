...
func (v Values) ApplyDefaultValues() error {
	values := FeatureNetwork.GetDefaultValues(CalicoChartName)
...
func (v Values) ApplyAnnotations(annotations types.Annotations) error {
	...
	installation := v["installation"].(map[string]any)
	calicoNetwork := installation["calicoNetwork"].(map[string]any)
	ipPools := calicoNetwork["ipPools"].([]map[string]any)
