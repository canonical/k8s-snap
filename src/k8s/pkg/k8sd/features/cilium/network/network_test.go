...
func TestNetworkMountPropagationType(t *testing.T) {
	testenv.WithState(t, func(ctx context.Context, s state.State) {
		t.Run("failedGetMountSys", func(t *testing.T) {
			...
			cilium_network.GetMountPropagationType = func(path string) (utils.MountPropagationType, error) {
				return "", mountErr
			}
			helmM := &helmmock.Mock{}
			snapM := &snapmock.Snap{
		...
		t.Run("MountPropagationPrivate", func(t *testing.T) {
			...
			cilium_network.GetMountPropagationType = func(_ string) (utils.MountPropagationType, error) {
				return utils.MountPropagationPrivate, nil
			}
			helmM := &helmmock.Mock{}
			snapM := &snapmock.Snap{
