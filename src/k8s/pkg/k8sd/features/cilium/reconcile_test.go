...
func TestReconcile(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		// given
		addrGetter   cilium.AddressGetter
		...
		{
			name:       "HappyPath",
			addrGetter: mockAddressGetter("https://127.0.0.2:4567"),
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			statuses, err := cilium.ApplyCilium(context.Background(), snapM, tc.addrGetter, tc.apiServer, tc.network, tc.gateway, tc.ingress, tc.annotations)
...
type mockAddressGetterImpl struct {
	address string
}

func (m *mockAddressGetterImpl) Address() *api.URL {
	u, _ := url.Parse(m.address)
	return &api.URL{URL: *u}
}

func mockAddressGetter(addr string) cilium.AddressGetter {
	return &mockAddressGetterImpl{
		address: addr,
	}
}
