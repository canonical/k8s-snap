...
func Etcd(snap snap.Snap, name, clientURL, peerURL string, clientURLs []string, extraArgs map[string]*string) error {
	...
	} else if err := os.WriteFile(filepath.Join(snap.EtcdDir(), "etcd.yaml"), b, 0o600); err != nil {
	...
	} else if err := os.WriteFile(filepath.Join(snap.EtcdDir(), "register.yaml"), b, 0o600); err != nil {
		return fmt.Errorf("failed to write register.yaml file for name=%q address=%q: %w", name, peerURL, err)
	}

	if _, err := snaputil.UpdateServiceArguments(snap, "etcd", map[string]string{
		"--state-dir": snap.EtcdDir(),
	}, nil); err != nil {
