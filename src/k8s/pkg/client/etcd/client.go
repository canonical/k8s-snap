...
func NewClient(pkiDir string, endpoints []string) (*Client, error) {
	client, err := newClient(pkiDir, endpoints)
	if err != nil {
...
func newClient(pkiDir string, endpoints []string) (*clientv3.Client, error) {
	certFile := fmt.Sprintf("%s/server.crt", pkiDir)
	keyFile := fmt.Sprintf("%s/server.key", pkiDir)
	caFile := fmt.Sprintf("%s/ca.crt", pkiDir)

	tlsConfig, err := loadTLSConfigFromPath(certFile, keyFile, caFile)
	if err != nil {
