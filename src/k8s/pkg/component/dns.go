...
func ReconcileDNSComponent(ctx context.Context, s snap.Snap, alreadyEnabled *bool, requestEnabled *bool, clusterConfig types.ClusterConfig) (string, string, error) {
	if vals.OptionalBool(requestEnabled, true) && vals.OptionalBool(alreadyEnabled, false) {
		...
		dnsIP, clusterDomain, err := UpdateDNSComponent(ctx, s, true, clusterConfig.Kubelet.GetClusterDomain(), clusterConfig.Kubelet.GetClusterDNS(), clusterConfig.Features.DNS.GetUpstreamNameservers())
		if err != nil {
			return "", "", fmt.Errorf("failed to refresh dns: %w", err)
		}
		return dnsIP, clusterDomain, nil
	} else if vals.OptionalBool(requestEnabled, false) {
		dnsIP, clusterDomain, err := UpdateDNSComponent(ctx, s, false, clusterConfig.Kubelet.GetClusterDomain(), clusterConfig.Kubelet.GetClusterDNS(), clusterConfig.Features.DNS.GetUpstreamNameservers())
		if err != nil {
