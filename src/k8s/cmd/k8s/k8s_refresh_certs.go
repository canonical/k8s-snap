...
func newRefreshCertsCmd(env cmdutil.ExecutionEnvironment) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if opts.externalCerts != "" {
				...
				_, err = client.RefreshCertificatesUpdate(ctx, certificates)
				if err != nil {
	...
	cmd.Flags().StringVar(&opts.externalCerts, "external-certificates", "", "path to a YAML file containing external certificate data in PEM format. If the cluster was bootstrapped with external certificates, the certificates will be updated.")
