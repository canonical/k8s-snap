...
k8s::remove::containers() {
  ...
  cat /proc/mounts | grep /var/lib/kubelet/plugins | grep "nfs[34]" | cut -f2 -d' ' | xargs -r -t umount -f || true
  cat /proc/mounts | grep /var/lib/kubelet/plugins | cut -f2 -d' ' | xargs -r -t umount || true
