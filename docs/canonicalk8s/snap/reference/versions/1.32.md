...
- Update LXD to {spellcheck}`v0.0.0-20241106165613-4aab50ec18c3`
...
- Move charm install files to {spellexception}`charm/howto/install`
...
- Add test for deploying NVIDIA {spellexception}`gpu-operator` through Helm chart
...
- Bump {spellexception}`golang.org/x/net` to v0.33.0
...
- Bump {spellexception}`golang.org/x/crypto` from v0.28.0 to v0.31.0
