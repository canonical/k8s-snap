...
- Honor AnnotationDisableSeparateFeatureUpgrades during node joins to prevent 
unintended upgrades.
...
- Version bumps
    ...
    - runc v1.3.5
    - MetalLB v0.14.9-ck8
- Improve DNS availability by introducing a CoreDNS rebalancing controller and 
applying best practices to scaling policies, pod distribution rules, and 
scheduling priorities
- Verify CoreDNS has a cluster IP assigned before reporting the cluster as ready
to improve cluster readiness detection
- Add ServiceArgsController to detect and remediate service argument drift,
automatically restarting affected services.
...
- Version bumps
    ...
    - SQLite v3.50.2
    - Microcluster v2.2.1 
- Bump Go and Python dependencies to address CVEs 
([#2483](https://github.com/canonical/k8s-snap/pull/2483))
- Remove stale src/k8s/go.mod reference from update scripts 
- Use etcd learner mode during node join to prevent quorum loss 
([#2515](https://github.com/canonical/k8s-snap/pull/2515))
- Pin all GitHub Actions dependencies to SHA hashes to satisfy OSSF Scorecard 
Pinned-Dependencies requirements 
([#2507](https://github.com/canonical/k8s-snap/pull/2507))
- Pin LXD to 5.21/stable for core20 snap builds 
- Update Trivy to a known-safe version 0.69.3 
- Add how to revert a failed upgrade to our docs 
- Apply fix to ensure snap services are running after a revert 
([#2423](https://github.com/canonical/k8s-snap/pull/2423))
- Add wait_for_pods_ready utility and integrate it into upgrade tests 
([#2395](https://github.com/canonical/k8s-snap/pull/2395))
- Update Cilium agent and operator to use static image tags 
([#2325](https://github.com/canonical/k8s-snap/pull/2325))
- Update CIS compliance audit docs with etcd controls 
- Add k8s-dqlite deprecation warning on bootstrap 
([#2340](https://github.com/canonical/k8s-snap/pull/2340))
- Add node join revert logic for `k8sd` 
([#2337](https://github.com/canonical/k8s-snap/pull/2337))
- Apply fix to disable volume resize for local-storage 
([#2248](https://github.com/canonical/k8s-snap/pull/2248))
- Fix Invalid `iproute2` JSON output in arm64 
([#1821](https://github.com/canonical/k8s-snap/pull/1821))
- Add Kubernetes version to node version annotation 
([#2159](https://github.com/canonical/k8s-snap/pull/2159))
- Add CNCF conformance tests 
([#2066](https://github.com/canonical/k8s-snap/pull/2066))
- Add support for installing from channel and revision for promotion upgrade 
tests 
- Apply fix to CLI logic to only require the
`--k8s-dqlite-state-dir`/ `--state-dir` if the corresponding
`--skip-<k8sd/k8s-dqlite>` flag is not set
- Update config used in EPA docs 
- Sanitize not-applicable feature gates in configure hook 
([#2023](https://github.com/canonical/k8s-snap/pull/2023))
