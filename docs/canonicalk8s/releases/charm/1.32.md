...
- Move shared COS-tokens to app databag to better handle losing a k8s leader 
unit ([#652](https://github.com/canonical/k8s-operator/pull/652))
- Add support for configuring COS observability for the managed-etcd service
- Add check to ensure datastore type is discovered when not explicitly set in 
the config 
- Pin ruff to v0.12.12 for formatting and linting 
- Add a k8s-dqlite deprecation warning to install 
