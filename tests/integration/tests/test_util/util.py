...
def create_gofips_patch(resource_def: dict) -> List[dict]:
    """
    Create JSON patch to add GOFIPS=1 environment variable to all containers.

    Args:
        resource_def: resource definition dict from kubectl get
...
def patch_resource_with_gofips(
    instance: harness.Instance,
    namespace: str,
    resource_type: str,
    name: str,
    ...
    """
    Patch a resource to add GOFIPS=1 environment variable to all containers.

    Args:
        instance: instance on which to execute patch
    ...
    # Create patch
    patches = create_gofips_patch(resource_def)
        ...
        for pod in pods_data.get("items", []):
            ...
            for container_status in container_statuses:
                ...
                if "CrashLoopBackOff" in reason or "Error" in reason:
                    LOG.info(f"Pod {pod_name} is in error state: {reason} - {message}")
