...
def test_embedded(instances: List[harness.Instance]):
    ...
    cluster_node.exec(["k8s", "bootstrap", "--file", "/root/config.yaml"])
    util.wait_for_dns(cluster_node)
    util.wait_for_network(cluster_node)
